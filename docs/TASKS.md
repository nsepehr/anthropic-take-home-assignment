...
| 5k  | `26-modal-text`          | 5     | P1       | done   | agent                    | 23         | Chat walkthrough modal reworded to the chat experience; trigger link renamed                   |
...
- 2026-08-27 — 26-modal-text → review (agent): the chat walkthrough modal is now "What happens during the chat experience" — four steps in the human's words (extension to Claude / the Claude Code UI, with @-tagging of features, requirements, intents and systems → the AI gets the context and attacks the work → it checks the change against your intent before applying, transcripts becoming a short user-verified summary of intent → human intent, product requirements and features living side by side as a forever capture); the trigger link under a scoped reply is now "See how the chat experience works ›" (`client/src/features/chat/ApplyDemo.tsx`, tests in `ApplyDemo.test.tsx` + `ChatDrawer.test.tsx`); footer sentence and layout unchanged; the now-unused `.apply-demo-pill` rule dropped from `chat.css`. Copy only — no behaviour, no seed change.
- 2026-08-27 — 26-modal-text → done (human): modal copy describes the chat experience
