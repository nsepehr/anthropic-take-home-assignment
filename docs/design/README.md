...
- Base rules and classes (`.btn*`, `.tag*`, `.card*`, `.seg*`, `.hr`, `.text-muted`) live in `client/src/styles/base.css`; `index.css` wires fonts, xyflow css, tokens, base and the `.sb` scrollbar utility. `main.tsx` imports only `index.css`.
